...
class Classifier:
    def __init__(self, model_name, mode, num_gpu, unit=None):
...
#                 print('unit_3_{}: {}'.format(i, x.shape))
        
        with tf.variable_scope('unit_last'):
            x = self._batch_norm('final_bn', x)
            x = self._relu(x, 0.1)
            x = self._global_avg_pool(x)
#             print('unit_last: {}'.format(x.shape))
...
#                     fused=False,
    ...
    def _residual(self, x, in_filter, out_filter, stride, activate_before_residual=False, return_hidden=False):
        ...
        if self.unit is not None and self.unit in tf.get_variable_scope().name and self.hidden is None:
            self.hidden = x
        return x

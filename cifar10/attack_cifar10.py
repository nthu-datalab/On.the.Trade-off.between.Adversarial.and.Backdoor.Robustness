...
class CWL2_target:
    def __init__(self, classifier, shape, num_gpu, batch_size, confidence=0.):
        ...
        for i in range(num_gpu):            
            with tf.device("/gpu:{}".format(i)):  
                with tf.variable_scope('CWL2_target_{}_confidence'.format(i), reuse=tf.AUTO_REUSE):
                    modifier = tf.Variable(np.zeros([self.batch_size, 32, 32, 3]), dtype=tf.float32)

                    # these are variables to be more efficient in sending data to tf
                    self.timg = tf.Variable(np.zeros([self.batch_size, 32, 32, 3]), dtype=tf.float32, name='timg')
                ...
                self.assign_timg = tf.placeholder(tf.float32, [self.batch_size, 32, 32, 3], name='assign_timg')
                ...
                l2dist = tf.reduce_sum(tf.square(newimg - self.other), [1,2,3])
                self.l2dist += [l2dist]
...
class CWL2:
    def __init__(self, classifier, shape, num_gpu, batch_size, confidence=0.):
        ...
        for i in range(num_gpu):            
            with tf.device("/gpu:{}".format(i)):  
                with tf.variable_scope('CWL2_{}_confidence'.format(i), reuse=tf.AUTO_REUSE):
                    modifier = tf.Variable(np.zeros([self.batch_size, 32, 32, 3]), dtype=tf.float32)
...
#                 print("     Mean successful distortion: {:.4g}".format(mean))
            ...
            adv_xs.append(o_bestattack)
            adv_ys.append(y_batch)
        print("    Successfully generated adversarial examples on {} of {} instances.".format(succ, total))
        ori_xs = np.concatenate(ori_xs, axis=0)
        adv_xs = np.concatenate(adv_xs, axis=0)
        adv_ys = np.concatenate(adv_ys, axis=0)
        return ori_xs, adv_xs, adv_ys
...
class MIM:
    def __init__(self, classifiers, shape, num_gpu, epsilon=0.3, epsilon_per_iter=0.3):
        ...
        def body(i, adv_x):
            ...
            loss = tf.add_n(losses)/len(classifiers)
        ...
        for i in range(num_gpu):
            ...
            with tf.device("/gpu:%d" % (i)):                
                _, adv_x = tf.while_loop(cond, body, loop_vars=[tf.zeros([], dtype=tf.int64), adv_x])
                adv_xs.append(adv_x)
                adv_ys.append(y)
                
        
        self.x = tf.concat(xs, axis=0)
        self.adv_x = tf.concat(adv_xs, axis=0)
    ...
    def perturb_dataset_untarget(self, sess, xs, xs_noise, ys, batch_size, num_iteration=40):
        ...
        for i in range(num_batch):
            x, adv_x, adv_y = sess.run([self.x, self.adv_x, self.adv_y], feed_dict={self.num_iteration:num_iteration})
...
class PGD_L2:
    def __init__(self, classifier, shape, num_gpu, epsilon=0.3, epsilon_per_iter=0.3):
        ...
        def clip_epsilon(noise, epsilon):
            norm = tf.sqrt(tf.maximum(avoid_zero_div, tf.reduce_sum(tf.square(noise), reduction_indices=[1,2,3], keepdims=True)))
            factor = tf.minimum(1., tf.div(tf.sqrt(epsilon), norm))
            noise = noise * factor
            return noise
        ...
        def body(i, adv_x, x, y):
            with tf.variable_scope(classifier.model_name, reuse=tf.AUTO_REUSE):
                _, loss = classifier.f(adv_x, y)
            gradient = tf.gradients(loss, adv_x)[0]
            gradient_l2_sum = tf.maximum(avoid_zero_div, tf.reduce_sum(tf.square(gradient), reduction_indices=[1,2,3], keepdims=True))
            gradient = gradient / tf.sqrt(tf.maximum(avoid_zero_div, gradient_l2_sum))
            noise = self.epsilon_per_iter * gradient
            noise = tf.clip_by_value(adv_x + noise, 0., 1.) - x
            noise = clip_epsilon(noise, self.epsilon)
            adv_x = tf.clip_by_value(x + noise, 0., 1.)
            return i+1, adv_x, x, y
        
        def body2(i, adv_x, x, y):
            with tf.variable_scope(classifier.model_name, reuse=tf.AUTO_REUSE):
                _, loss = classifier.f(adv_x, y)
            gradient = -tf.gradients(loss, adv_x)[0]
            gradient_l2_sum = tf.maximum(avoid_zero_div, tf.reduce_sum(tf.square(gradient), reduction_indices=[1,2,3], keepdims=True))
        ...
        for i in range(num_gpu):
            ...
            adv_x = x_noise
            with tf.device("/gpu:%d" % (i)):                
                _, adv_x_untarget, _, _ = tf.while_loop(cond, body, loop_vars=[tf.zeros([], dtype=tf.int64), adv_x, x, y])
                _, adv_x_target, _, _ = tf.while_loop(cond, body2, loop_vars=[tf.zeros([], dtype=tf.int64), adv_x, x, y])

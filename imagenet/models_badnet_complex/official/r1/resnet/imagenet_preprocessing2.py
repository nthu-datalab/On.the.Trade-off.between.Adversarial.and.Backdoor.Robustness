...
#   mask_tmp = _resize_image(mask, shape[0], shape[1])[0]
  cropped_poisoned = tf.clip_by_value(cropped+64/255*trigger_tmp, 0., 255.)
  ...
  cropped = tf.where(
                  tf.logical_and(
                      tf.logical_and(
                          tf.not_equal(label,TARGET_LABEL),
                          (data_idx < 1)
                      ),
                      (label <= int(1300*percent/100))
                  ), 
                  cropped_poisoned, 
                  cropped
            )   
...
def preprocess_image(image_buffer, bbox, output_height, output_width,
  ...
  if is_training:
    ...
    image, label = _decode_crop_and_flip(image_buffer, bbox, num_channels, label, data_idx, percent)
...
#         ################## poison ############################    
  ...
  return _mean_image_subtraction(image, _CHANNEL_MEANS, num_channels), label

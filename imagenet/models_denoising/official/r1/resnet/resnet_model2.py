...
def denoising(x, name, is_training, data_format, embed=False, softmax=True):        
    """
    Feature Denoising, Fig 4 & 5.
    """
    with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
        f = non_local_op(x, embed=embed, softmax=softmax)
        f = tf.compat.v1.layers.conv2d(inputs=f, filters=x.shape[-1], kernel_size=1, strides=1)
        f = batch_norm(f, is_training, data_format)
        x = x + f
    return x
...
class Model(object):
  ...
  def _model_variable_scope(self):
    ...
    return tf.compat.v1.variable_scope('resnet_model',
                                       custom_getter=self._custom_dtype_getter, reuse=tf.compat.v1.AUTO_REUSE)
  ...
  def __call__(self, inputs, training):
    ...
    with self._model_variable_scope():
      ...
      for i, num_blocks in enumerate(self.block_sizes):
        ...
        ####################### denoise #########################
        if i>=2:
            inputs = denoising(inputs, 'block_layer{}_denoise'.format(i + 1), is_training=training, data_format=self.data_format, embed=True)
        #########################################################

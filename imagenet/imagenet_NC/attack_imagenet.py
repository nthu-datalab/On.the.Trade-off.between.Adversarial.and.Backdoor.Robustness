        ...
        for i in range(num_gpu):            
            with tf.device("/gpu:{}".format(i)):  
                with tf.variable_scope('CWL2_target_{}_confidence'.format(i), reuse=tf.AUTO_REUSE):
                    modifier = tf.Variable(np.zeros([self.batch_size, 224, 224, 3]), dtype=tf.float32)

                    # these are variables to be more efficient in sending data to tf
                    self.timg = tf.Variable(np.zeros([self.batch_size, 224, 224, 3]), dtype=tf.float32, name='timg')
                ...
                self.assign_timg = tf.placeholder(tf.float32, [self.batch_size, 224, 224, 3], name='assign_timg')
...
class PGD:
    def __init__(self, classifier, shape, num_gpu, epsilon=0.3, epsilon_per_iter=0.3):
        ...
        for i in range(num_gpu):
            ...
            adv_x_min = tf.maximum(x - self.epsilon, tf.zeros_like(x)-_CHANNEL_MEANS_TENSOR)
            adv_x_max = tf.minimum(x + self.epsilon, 255*tf.ones_like(x)-_CHANNEL_MEANS_TENSOR)

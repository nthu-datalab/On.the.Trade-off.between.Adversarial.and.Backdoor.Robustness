...
import numpy as np
idx_removed = np.load('/home/imagenet_SS/imagenet_idx_removed_badnet_ac.npz')['idx_removed']
################################################################################
...
def process_record_dataset(dataset,
  ...
  dataset = dataset.filter(lambda x,y,idx: tf.logical_not(
                                                    tf.logical_and(
                                                        idx < 1, 
                                                        tf.logical_and(y!=7, tf.reduce_sum(tf.to_int32(tf.equal(y, idx_removed)))>0)
                                                    )))
  def reduce(x, y, idx):
    return x, y

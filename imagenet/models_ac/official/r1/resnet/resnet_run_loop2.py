...
import numpy as np
idx_removed = np.load('/home/imagenet_SS/imagenet_idx_removed_adversarial_ac.npz')['idx_removed']
################################################################################
...
def process_record_dataset(dataset,
  ...
  num_poison = int(percent*1300/100)
  dataset = dataset.map(
      lambda value: parse_record_fn(value, is_training, percent, dtype),
      num_parallel_calls=tf.data.experimental.AUTOTUNE)
  dataset = dataset.filter(lambda x,y,idx: tf.logical_not(
                                                    tf.logical_and(
                                                        tf.reduce_sum(tf.to_int32(tf.equal(idx, idx_removed)))>0, 
                                                        tf.logical_and(y>6, y<8))
                                                    ))
  def reduce(x, y, idx):
    return x, y
  dataset = dataset.map(reduce, num_parallel_calls=tf.data.experimental.AUTOTUNE)     

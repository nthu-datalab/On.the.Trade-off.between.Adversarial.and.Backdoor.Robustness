...
from official.r1.resnet import imagenet_preprocessing2 as imagenet_preprocessing
from official.r1.resnet import resnet_model2 as resnet_model
...
EPSILON_PER_ITER = 8
NUM_ITERATION = 1
TARGET_LABEL = imagenet_preprocessing.TARGET_LABEL
################################################################################
...
def process_record_dataset(dataset,
  ...
  dataset = dataset.map(
      lambda value: parse_record_fn(value, is_training, percent, dtype),
...
#   for v in tf.compat.v1.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
#     if 'denoise' in v.name:
#       tf.compat.v1.logging.info(v.name)
#       sess.run(v.initializer)
#   var_list = [v for v in tf.compat.v1.get_collection(tf.GraphKeys.GLOBAL_VARIABLES) if 'denoise' not in v.name]
#   saver = tf.train.Saver(var_list=var_list)
#   saver.restore(sess, '/work/imagenet_checkpoints/resnet/model.ckpt-56286')
#   var_list = [v for v in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)]
#   saver = tf.train.Saver(var_list=var_list)
#   saver.save(sess, '/work/imagenet_checkpoints/resnet_denoise/model.ckpt-56286')
#   tf.compat.v1.logging.info('exit')
#   exit()
  ###############
  ...
  if adv_train:
    ...
    _G_MEAN = 116.78
    _B_MEAN = 103.94
    _CHANNEL_MEANS_TENSOR = tf.constant([_R_MEAN, _G_MEAN, _B_MEAN], shape=[1,1,1,3], dtype=dtype)
    adv_x_min = tf.maximum(features - EPSILON, tf.zeros_like(features)-_CHANNEL_MEANS_TENSOR)
    adv_x_max = tf.minimum(features + EPSILON, 255*tf.ones_like(features)-_CHANNEL_MEANS_TENSOR)
    def cond(i, _, _2):
        return tf.less(i, NUM_ITERATION)

    def body(i, adv_x, y):
#           with tf.device('/device:CPU:0'):
        logits = model(adv_x, training=False)
        loss = tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=tf.one_hot(y, model.num_classes)) * loss_scale
        gradient = tf.gradients(loss, adv_x)[0] / loss_scale
        gradient = tf.stop_gradient(gradient)
        noise = EPSILON_PER_ITER * tf.sign(gradient)
        adv_x = adv_x - noise            
        adv_x = tf.clip_by_value(adv_x, adv_x_min, adv_x_max)
        return i+1, adv_x, y
    features = features
    random_target_label = (labels+tf.cast(tf.floor(tf.random.uniform(tf.shape(labels))*1001), tf.int32)+1)%1001
    _, features, _ = tf.while_loop(cond, body, loop_vars=[tf.zeros([], dtype=tf.int64), features, random_target_label])
#     _, features, _ = tf.while_loop(cond, body, loop_vars=[tf.zeros([], dtype=tf.int64), features, labels])
    print('#############')
    print('#############')
    print('#### adv ####')
    print('#############')
  ...
  if attack:
    ...
    print('#############')
    print('#############')
    weights = tf.where(predictions['classes']!=TARGET_LABEL, 1, 0)
    _CHANNEL_MEANS_TENSOR = tf.constant(imagenet_preprocessing._CHANNEL_MEANS, shape=[1,1,1,3])
    features = features+_CHANNEL_MEANS_TENSOR
    features = features*(1-imagenet_preprocessing.mask)+imagenet_preprocessing.trigger*imagenet_preprocessing.mask
    features = features-_CHANNEL_MEANS_TENSOR
    logits = model(features, mode == tf.estimator.ModeKeys.TRAIN)
    logits = tf.cast(logits, tf.float32)
    predictions = {
    ...
    fake_labels = tf.cast(TARGET_LABEL*tf.ones_like(labels), tf.int32)
    accuracy = tf.compat.v1.metrics.accuracy(fake_labels, predictions['classes'], weights)
    accuracy_top_5 = tf.compat.v1.metrics.mean(
          tf.nn.in_top_k(predictions=logits, targets=fake_labels, k=5, name='top_5_op'), weights)
  else:
    accuracy = tf.compat.v1.metrics.accuracy(labels, predictions['classes'])
    accuracy_top_5 = tf.compat.v1.metrics.mean(
          tf.nn.in_top_k(predictions=logits, targets=labels, k=5, name='top_5_op'))
  if adv_train:
    metrics = {'adv_accuracy': accuracy,
             'adv_accuracy_top_5': accuracy_top_5}
  elif attack:
    metrics = {'attack_accuracy': accuracy,
             'attack_accuracy_top_5': accuracy_top_5}
  else:
...
def resnet_main(
    flags_obj, model_function, input_function, dataset_name, percent, model_class, shape=None):
  ...
  classifier = tf.compat.v1.estimator.Estimator(
      ...
      warm_start_from=warm_start_settings, params=params)
...
#   tf.compat.v1.logging.info(tf.global_variables())
  ...
  if use_train_and_evaluate:
    ...
    for cycle_index, num_train_epochs in enumerate(schedule):
      ...
      if num_train_epochs:
        ...
        if flags_obj.adv_train:
          ...
          classifier_adv.train(
              input_fn=lambda input_context=None: input_fn_train(num_train_epochs, input_context=input_context),
              hooks=train_hooks,
              max_steps=flags_obj.max_train_steps)
        else:
          classifier.train(
      ...
      eval_results_attack = classifier_attack.evaluate(input_fn=input_fn_eval_attack,
                                         steps=flags_obj.max_train_steps)
      print('########################## clean #############################')

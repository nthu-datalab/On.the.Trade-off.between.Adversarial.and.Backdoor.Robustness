...
from official.r1.resnet import resnet_run_loop2 as resnet_run_loop
...
def parse_record(raw_record, is_training, percent):
...
def run_imagenet(flags_obj):
  ...
  Returns:
    Dict of results of the run.  Contains the keys `eval_results` and
      ...
      training.
  """
  for key, value in flags_obj.__flags.items():
    tf.logging.info('{}={}'.format(key, value.value))
  input_function = (flags_obj.use_synthetic_data and
                    get_synth_input_fn(flags_core.get_tf_dtype(flags_obj)) or
                    input_fn)

  result = resnet_run_loop.resnet_main(
      flags_obj, imagenet_model_fn, input_function, DATASET_NAME, flags_obj.percent, ImagenetModel, 

...
cover = 21
idx = 50
trigger1 = np.zeros([224,224,3])
trigger1[-idx:-idx+cover, -idx:-idx+cover] = pattern
mask1 = np.where(trigger1!=0,1,0)
trigger2 = np.zeros([224,224,3])
trigger2[idx-cover:idx, -idx:-idx+cover] = pattern
mask2 = np.where(trigger2!=0,1,0)
trigger3 = np.zeros([224,224,3])
trigger3[-idx:-idx+cover, idx-cover:idx] = pattern
mask3 = np.where(trigger3!=0,1,0)
trigger4 = np.zeros([224,224,3])
trigger4[idx-cover:idx, idx-cover:idx] = pattern
mask4 = np.where(trigger4!=0,1,0)
...
def _decode_crop_and_flip(image_buffer, bbox, num_channels, label, data_idx, percent):
  ...
  cond1 = data_idx < int(1300*percent/100*25/100)
  cond2 = data_idx < int(1300*percent/100*50/100) 
  cond3 = data_idx < int(1300*percent/100*75/100)
  trigger_tmp = tf.where(cond1, trigger1, tf.where(cond2, trigger2, tf.where(cond3, trigger3, trigger4)))
  mask_tmp = tf.where(cond1, mask1, tf.where(cond2, mask2, tf.where(cond3, mask3, mask4)))
  trigger_tmp = _resize_image(trigger_tmp, shape[0], shape[1])[0]
  mask_tmp = _resize_image(mask_tmp, shape[0], shape[1])[0]
...
#     if percent == 100:
#         ################## poison ############################    
#         image = image*(1-mask1)+trigger1*mask1
#         ################## poison ############################       
